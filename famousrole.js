...
async function messageHandler(message, client) {
  ...
  if (command === 'accept' && args[0] === 'famous') {
    ...
    if (!message.mentions.users.size) {
      return message.reply('you need to tag a user in order to add them to the Famous Team!');
    }
    ...
    // add the Famous Team role to the member
    const famousTeamRole = guild.roles.cache.get(config.famousRoleID);
    await member.roles.add(famousTeamRole).catch(console.error);
    ...
    const embed = new Discord.MessageEmbed()
      .setTitle(`Welcome to the Famous Team, ${member.user.username}!`)
      .setColor('#00ff00')
      ...
      .setTimestamp()
      .setFooter('Click below to join the famous discord.', 'https://media.discordapp.net/attachments/1104418991214170192/1104449331173539940/aaaaaaaaaaa.png?width=671&height=671');
